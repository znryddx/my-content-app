...
SYSTEM = (
    ...
    "【时令借势硬约束】下方【临近节日/节气提醒】若列出临近节点，凡涉及时令/节气/礼赠/氛围的版块"
    "（尤其是 solar 单元）必须主动结合这些节点借势，临近 1–3 天内的节日/节气要显式点名并给出借势角度，"
    "绝不可只写笼统的『节气第几日』而忽略更近的节日（如七夕、中秋、情人节等）。"
)

BATCH_SIZE = 1  # 每分类单独生成：避免免费模型在批量时掏空前几个分类的 body；靠分类间间隔+重试避开限流


# ---------- 节日 / 节气提醒（零依赖静态表，每年更新一次即可） ----------
# 24 节气（公历近似，±1 天；用于推算「当前节气第 N 日」）
_TERMS = [
    ("2026-01-05", "小寒"), ("2026-01-20", "大寒"), ("2026-02-04", "立春"),
    ("2026-02-19", "雨水"), ("2026-03-05", "惊蛰"), ("2026-03-20", "春分"),
    ("2026-04-05", "清明"), ("2026-04-20", "谷雨"), ("2026-05-05", "立夏"),
    ("2026-05-21", "小满"), ("2026-06-05", "芒种"), ("2026-06-21", "夏至"),
    ("2026-07-07", "小暑"), ("2026-07-22", "大暑"), ("2026-08-07", "立秋"),
    ("2026-08-23", "处暑"), ("2026-09-07", "白露"), ("2026-09-23", "秋分"),
    ("2026-10-08", "寒露"), ("2026-10-23", "霜降"), ("2026-11-07", "立冬"),
    ("2026-11-22", "小雪"), ("2026-12-07", "大雪"), ("2026-12-22", "冬至"),
    ("2027-01-05", "小寒"), ("2027-01-20", "大寒"),
]
# 主要节日 / 营销节点（公历；农历节日已折算为 2026 年公历日）
_FESTIVALS = [
    ("2026-01-01", "元旦", "新年开篇·愿景盘点"),
    ("2026-02-14", "情人节", "浪漫·礼赠"),
    ("2026-02-17", "春节", "团圆·年礼·家文化"),
    ("2026-03-03", "元宵", "灯会·团圆收尾"),
    ("2026-03-08", "妇女节", "她经济·女性礼赠"),
    ("2026-03-20", "龙抬头", "开运·春理发"),
    ("2026-04-05", "清明", "踏青·慎终追远（亦节气）"),
    ("2026-05-01", "劳动节", "假期·出行消费"),
    ("2026-05-04", "青年节", "年轻化·潮流"),
    ("2026-06-01", "儿童节", "亲子·童趣"),
    ("2026-06-19", "端午", "安康·礼粽"),
    ("2026-08-19", "七夕", "东方定情·礼赠高峰"),
    ("2026-08-27", "中元", "慎终·静思（宜轻营销）"),
    ("2026-09-10", "教师节", "感恩·师礼"),
    ("2026-09-25", "中秋", "团圆·月礼·家文化"),
    ("2026-10-01", "国庆", "长假·家国·出游"),
    ("2026-10-19", "重阳", "敬老·登高·秋礼"),
    ("2026-11-11", "双十一", "电商大促·转化"),
    ("2026-11-26", "感恩节", "感恩·西式礼赠"),
    ("2026-12-24", "平安夜", "节日氛围·礼赠"),
    ("2026-12-25", "圣诞", "节日氛围·礼赠"),
    ("2027-01-01", "元旦", "辞旧迎新·愿景"),
]


def festival_hint(today):
    """返回『今日及未来 3 天』的节日/节气提醒，注入给 LLM 以驱动时令借势。"""
    if isinstance(today, str):
        today = datetime.date.fromisoformat(today)
    lines = []
    # 当前节气第几日
    cur = None
    for d, name in _TERMS:
        dd = datetime.date.fromisoformat(d)
        if dd <= today:
            cur = (dd, name)
    if cur:
        idx = (today - cur[0]).days + 1
        lines.append("今天是 %s第%d日（%s，节气）。" % (cur[1], idx, cur[0].strftime("%m/%d")))
    # 未来 3 天窗口（含今日）内的节点
    end = today + datetime.timedelta(days=3)
    up = []
    for d, name, note in _FESTIVALS:
        dd = datetime.date.fromisoformat(d)
        if today <= dd <= end:
            delta = (dd - today).days
            when = "今日" if delta == 0 else ("明日" if delta == 1 else ("后天" if delta == 2 else "大后天"))
            up.append((delta, "%s %s（%s）—%s" % (dd.strftime("%m/%d"), name, note, when)))
    for d, name in _TERMS:
        dd = datetime.date.fromisoformat(d)
        if today < dd <= end:  # 仅提示尚未到来的节气
            delta = (dd - today).days
            when = "明日" if delta == 1 else ("后天" if delta == 2 else "大后天")
            up.append((delta, "%s %s（节气）—%s" % (dd.strftime("%m/%d"), name, when)))
    if up:
        up.sort()
        lines.append(
            "【临近节日/节气提醒】未来数日内有以下节点，请在涉及时令/节气/礼赠/氛围的版块"
            "（尤其 solar 单元）主动借势，临近 1–3 天的务必显式点名，不得只写笼统节气而忽略临近节日："
        )
        lines.append("；".join(t for _, t in up))
    return "\n".join(lines)
